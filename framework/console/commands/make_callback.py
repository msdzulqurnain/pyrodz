...
class MakeCallback(Command):
    SUFFIX = "Handler"

    def handle(self, *args):
        if not args:
            print()
            print(Style.warn("Usage: python3 pyrodz make:callback Menu"))
            return
        ...
        callback_name = raw_name.replace(self.SUFFIX, "").lower()
        ...
        method_code = (
            f'\n    async def {method_name}(self, client, callback_query):'
            f'\n        await callback_query.message.edit_text(f"You pressed {method_name}!")'
            f'\n'
        )
        ...
        self.add_route(name, callback_name, method_name)

        print()
        print(Style.success(f"Callback {callback_name} created successfully"))
        print(Style.info(f'Location: {str(path)}'))

    def add_route(self, class_name, callback_name, method_name):
        ...
        route_line = f'Route.callback("{callback_name}", {class_name}.{method_name})'

        content = route_path.read_text(encoding="utf-8")

        if route_line in content:
            if not self.confirm(Style.info(f'Route "{callback_name}" already exists. Overwrite?')):
                print()
